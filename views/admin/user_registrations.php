        ...
        function displayUsers(users) {
            ...
            tbody.innerHTML = users.map(user => `
                <tr class="hover:bg-slate-50/70 transition-colors">
                    <td class="px-6 py-4"><div class="flex items-center gap-3"><img src="${user.profile_photo || '../../assets/images/default-avatar.svg'}" onerror="this.onerror=null;this.src='../../assets/images/default-avatar.svg';" class="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0" alt="${user.full_name}"><div><div class="font-bold text-slate-900 text-xs">${user.full_name}</div><div class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">ID: ${user.user_id}</div></div></div></td>
