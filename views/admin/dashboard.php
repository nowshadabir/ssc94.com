...
$pageSubtitle = "SSC Batch '94 — Admin Dashboard";
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="flex min-h-screen">
        ...
        <div class="p-6 lg:p-8 space-y-8 overflow-y-auto">
                ...
                <div class="stat-card bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                    <div class="flex items-start justify-between mb-3">
                        <div class="w-10 h-10 bg-green-50 text-green-600 rounded-xl flex items-center justify-center"><i
                                data-lucide="banknote" class="w-5 h-5"></i></div>
                    </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="p-6" id="divisionBars">
                    <div class="flex items-center justify-center h-24 text-slate-300"><i data-lucide="loader-2"
                            class="animate-spin w-6 h-6"></i></div>
                </div>
            </div>
                    ...
                    <form id="reunionForm" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div class="sm:col-span-1 lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div><label
                                    ...
                                    Menu / Feast</label><textarea name="food_menu" rows="3"
                            ...
                            <div><label
                                    ...
                                    / Entertainment</label><textarea name="activities" rows="3"
                        ...
                        <div class="sm:col-span-2 lg:col-span-3 flex items-center gap-3">
                            <button type="submit" id="saveBtn"
                                class="bg-slate-900 text-white px-8 py-2.5 rounded-xl font-bold text-sm hover:bg-black transition flex items-center gap-2"><i
                                    data-lucide="save" class="w-4 h-4"></i> Save Reunion Details</button>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
                    ...
                    <span id="regCountBadge"
                        class="ml-auto text-[11px] bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded-full hidden"></span>
                </div>
                <div id="regLoading" class="hidden p-10 flex justify-center"><i data-lucide="loader-2"
                        class="w-7 h-7 animate-spin text-slate-300"></i></div>
                <div id="regTableWrap" class="overflow-x-auto">
                    <table class="w-full text-left min-w-[700px]">
                        ...
                        <tbody id="regTableBody" class="divide-y divide-slate-50 text-sm"></tbody>
                    </table>
                </div>
                <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
                    <p class="text-xs text-slate-500">Showing <strong id="regRange" class="text-slate-800">—</strong> of
                        <strong id="regTotal" class="text-slate-800">—</strong>
                    </p>
                    <div class="flex items-center gap-2">
                        <button id="prevBtn" onclick="changePage(-1)" disabled
                            class="p-1.5 rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition"><i
    ...
    <div id="detailModal"
        ...
        <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div class="bg-slate-900 px-6 py-4 flex items-center justify-between shrink-0">
                <div class="flex items-center gap-3"><i data-lucide="ticket" class="w-5 h-5 text-yellow-400"></i><span
                        class="text-white font-bold text-sm uppercase tracking-wider">Registration Detail</span></div>
                <button onclick="closeModal()" class="text-slate-400 hover:text-white transition"><i data-lucide="x"
                        class="w-5 h-5"></i></button>
            </div>
            <div id="modalBody" class="overflow-y-auto p-6 space-y-5 text-sm"></div>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadStats() {
            try {
                ...
                container.innerHTML = Object.entries(divs).map(([name, count], i) => {
                    ...
                    return `<div class="flex items-center gap-3 mb-3 group"><span class="w-24 text-xs font-semibold text-slate-600 shrink-0 text-right">${name}</span><div class="flex-1 bg-slate-100 rounded-full h-5 overflow-hidden"><div class="div-bar h-full rounded-full flex items-center justify-end pr-2" style="width:${barW}%; background:${DIV_COLORS[i % DIV_COLORS.length]};"></div></div><span class="w-20 text-xs text-slate-500 font-bold shrink-0">${count} <em class="not-italic text-slate-400">(${pct}%)</em></span></div>`;
                }).join('');

                if (d.reunion) {
                    const r = d.reunion;
                    const form = document.getElementById('reunionForm');
                    form.title.value = r.title || ''; form.reunion_date.value = r.reunion_date || ''; form.reunion_time.value = r.reunion_time || ''; form.venue.value = r.venue || ''; form.venue_details.value = r.venue_details || ''; form.cost_alumnus.value = r.cost_alumnus || ''; form.cost_guest.value = r.cost_guest || ''; form.registration_deadline.value = r.registration_deadline || ''; form.food_menu.value = r.food_menu || ''; form.activities.value = r.activities || '';
                    if (form.status) form.status.value = r.status || 'active';
                    const badge = document.getElementById('reunionStatusBadge');
                    badge.classList.remove('hidden');
                    if (r.status === 'active') { badge.className = 'ml-auto text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-green-100 text-green-700'; badge.textContent = '🟢 Active'; }
                    else if (r.status === 'completed') { badge.className = 'ml-auto text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-700'; badge.textContent = '🔵 Completed'; }
                }
        ...
        document.getElementById('reunionForm').addEventListener('submit', async function (e) {
            ...
            btn.disabled = true; btn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline mr-1"></i> Saving…';
            lucide.createIcons();
            try {
                const res = await fetch('../../api/admin/reunion_update.php', { method: 'POST', body: new FormData(this) });
                const json = await res.json();
                status.classList.remove('hidden');
                if (json.success) { status.className = 'text-sm font-semibold text-green-600'; status.textContent = '✓ Saved successfully!'; }
                else { status.className = 'text-sm font-semibold text-red-600'; status.textContent = json.message || 'Save failed'; }
                setTimeout(() => status.classList.add('hidden'), 4000);
            } catch (err) { status.classList.remove('hidden'); status.className = 'text-sm font-semibold text-red-600'; status.textContent = 'Connection error'; }
            finally { btn.disabled = false; btn.innerHTML = '<i data-lucide="save" class="w-4 h-4 inline mr-1"></i> Save Reunion Details'; lucide.createIcons(); }
        });

        let currentPage = 1; const PER_PAGE = 20;
        ...
        async function loadRegistrations(page = 1) {
            currentPage = page; const tbody = document.getElementById('regTableBody'); const loader = document.getElementById('regLoading'); const tableW = document.getElementById('regTableWrap');
            loader.classList.remove('hidden'); tableW.classList.add('hidden');
            try {
                ...
                const regs = json.data.registrations; const meta = json.data.pagination; _regsCache = regs;
                if (regs.length === 0) { tbody.innerHTML = `<tr><td colspan="8" class="px-5 py-10 text-center text-slate-400 italic">No registrations yet.</td></tr>`; }
                else {
                    tbody.innerHTML = regs.map((r, idx) => {
                        ...
                        return `<tr class="hover:bg-slate-50/70 transition-colors"><td class="px-5 py-3"><div class="flex items-center gap-2.5"><img src="${avatar}" class="w-8 h-8 rounded-full object-cover border border-slate-200 shrink-0" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(r.full_name)}&background=e2e8f0&color=475569'"><div><div class="font-semibold text-slate-900 text-xs leading-tight">${r.full_name}</div><div class="text-slate-400 text-[10px]">${r.mobile}</div></div></div></td><td class="px-5 py-3"><span class="bg-indigo-50 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wider">${r.ticket_number}</span></td><td class="px-5 py-3 text-xs text-slate-600 font-medium">${r.tshirt_size}</td><td class="px-5 py-3 text-xs text-slate-700">${r.guest_count > 0 ? `<span class="font-bold">${r.guest_count}</span> guest${r.guest_count > 1 ? 's' : ''}` : '<span class="text-slate-400">None</span>'}</td><td class="px-5 py-3 text-xs font-bold text-slate-900">৳ ${parseFloat(r.total_amount).toLocaleString()}</td><td class="px-5 py-3">${BADGE[r.payment_status] ?? r.payment_status}</td><td class="px-5 py-3 text-[11px] text-slate-500">${new Date(r.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</td><td class="px-5 py-3 text-center"><button onclick="viewRegModal(${idx})" class="text-indigo-600 hover:text-indigo-800 text-[11px] font-bold border border-indigo-100 hover:border-indigo-300 px-3 py-1 rounded-lg transition">View</button></td></tr>`;
                    }).join('');
                }
                const start = (meta.current_page - 1) * meta.limit + 1; const end = Math.min(meta.current_page * meta.limit, meta.total_count);
                document.getElementById('regRange').textContent = `${start}–${end}`; document.getElementById('regTotal').textContent = meta.total_count; document.getElementById('regPage').textContent = meta.current_page;
                document.getElementById('prevBtn').disabled = meta.current_page <= 1; document.getElementById('nextBtn').disabled = meta.current_page >= meta.total_pages;
                document.getElementById('regCountBadge').textContent = meta.total_count + ' total'; document.getElementById('regCountBadge').classList.remove('hidden');
            } catch (e) { console.error('Reg load error', e); }
            finally { loader.classList.add('hidden'); tableW.classList.remove('hidden'); lucide.createIcons(); }
        }
        function changePage(dir) { loadRegistrations(currentPage + dir); }
        function viewRegModal(idx) {
            const r = _regsCache[idx]; if (!r) return;
            let guests = r.guests_data || [];
            if (typeof guests === 'string') {
                try { guests = JSON.parse(guests); } catch (e) { guests = []; }
            }
            const guestHtml = guests.length > 0 ? guests.map((g, i) => `<div class="flex items-center gap-3 bg-slate-50 rounded-lg px-4 py-2.5 border border-slate-100"><span class="text-xs font-bold text-slate-500 w-5">${i + 2}</span><div class="flex-1"><span class="font-semibold text-slate-800">${g.name || '—'}</span><span class="ml-2 text-slate-400 text-[11px]">${g.gender ?? ''}</span></div><span class="text-[11px] font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded">${g.tshirt ?? ''}</span></div>`).join('') : '<p class="text-slate-400 text-xs italic">No additional guests</p>';

            const totalAmount = parseFloat(r.total_amount || 0).toLocaleString();
            const regDate = r.created_at ? new Date(r.created_at).toLocaleString() : '—';

            document.getElementById('modalBody').innerHTML = `
                <div class="flex items-center gap-4 pb-4 border-b border-slate-100">
                    <img src="${r.profile_photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(r.full_name)}&background=0f172a&color=fff&size=120`}" class="w-16 h-16 rounded-full object-cover border-2 border-slate-200" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(r.full_name)}&background=0f172a&color=fff'">
                    <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    ${infoCell('Total Amount', '৳ ' + totalAmount, 'banknote')}
                    ...
                    ${infoCell('Registered', regDate, 'calendar')}
                </div>
                <div>
        ...
        function infoCell(label, value, icon) { return `<div class="bg-slate-50 rounded-xl p-3 border border-slate-100"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><i data-lucide="${icon}" class="w-3 h-3"></i>${label}</p><p class="text-sm font-semibold text-slate-800 truncate">${value}</p></div>`; }
